...
HOME = here::here()
source(glue("{HOME}/utils/R/bmk_io.R"))
...
generate_clone_table <- function(seu, features_file, sample.ID, locus.use) {
  ...
  X_df <- X_df %>% 
    mutate(cell_bc = colnames(X)[j], 
           clone_id = paste0("clone_", i),
           clone_bc = rownames(X)[i]) %>% 
    select(-i, -j) %>% 
    left_join(allele_df, by = "clone_bc")
  
  X_df <- X_df %>% 
    mutate(library = sample.ID, 
...
report_QC = function(cells.mrna, clone.table.file, sample.ID, locus.use, log_file = NULL) {
  # Read data
  ...
  clone_QC = c(
    X_df %>% filter(is_pass_mRNA_QC) %>% select(clone_id) %>% distinct() %>% nrow(),
    X_df %>% filter(is_pass_mRNA_QC & is_dominant) %>% select(clone_id) %>% distinct() %>% nrow(),
    X_df %>% filter(is_pass_mRNA_QC & is_dominant & is_edited) %>% select(clone_id) %>% distinct() %>% nrow(),
    X_df %>% filter(is_pass_mRNA_QC & is_dominant & is_edited & is_rare) %>% select(clone_id) %>% distinct() %>% nrow()
  )
...
AlleleStepwiseQCByCell = function(X_df, cells.mrna, freq.cutoff=1e-4, max.sample.N=3) {
  ...
  cells.bc = X_df %>% 
    pull(cell_bc) %>% unique() %>% intersect(., cells.mrna)
  cells.dominant = X_df %>% filter(is_dominant) %>% 
    pull(cell_bc) %>% unique() %>% intersect(., cells.mrna)
  cells.edited = X_df %>% filter(is_dominant & is_edited) %>% 
    pull(cell_bc) %>% unique() %>% intersect(., cells.mrna)
  cells.rare = X_df %>% 
...
AlleleStepwiseQCByAllele = function(X_df, cells.mrna, freq.cutoff=1e-4, max.sample.N=3) {
  # cells after stepwise QC
  clones.mrna = X_df %>% filter(cell_bc %in% cells.mrna) %>% 
    pull(clone_bc) %>% unique()
  clones.dominant = X_df %>% filter(cell_bc %in% cells.mrna) %>% 
    filter(is_dominant) %>% 
    pull(clone_bc) %>% unique()
  cells.edited = X_df %>% filter(cell_bc %in% cells.mrna) %>% 
    filter(is_dominant & is_edited) %>% 
    pull(clone_bc) %>% unique()
  cells.rare = X_df %>% filter(cell_bc %in% cells.mrna) %>% 
    mutate(is_rare = normalized_count < freq.cutoff & sample_count <= max.sample.N) %>% 
    filter(is_dominant & is_edited & is_rare) %>% 
    pull(clone_bc) %>% unique()
  # store results
